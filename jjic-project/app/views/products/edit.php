    ...
    <div class="card-group justify-content-center">
        ...
        <form action="" method="POST" enctype="multipart/form-data" class="mt-5 mb-5 w-50 mx-auto  p-5 shadow-lg mx-auto border-top border-bottom border-secondary">
            ...
            <div class="mb-2">
                ...
                <input class="form-control" value="<?= get_var('product_name', $row[0]->product_name)?>"type="text" name="product_name" placeholder="Product Name">
            </div>
            <div class="mb-2">
                <label for="" class="text-white">Product Description</label>
                <input class="form-control" value="<?= get_var('product_description', $row[0]->product_description)?>"type="text" name="product_description" placeholder="Product Description">
            </div>
            <div class="mb-2">
                <label for="" class="text-white">Product Price</label>
                <input class="form-control" value="<?= get_var('product_price', $row[0]->product_price)?>"type="text" name="product_price" placeholder="Product Price">
            </div>
            <div class="mb-2">
                <label for="" class="text-white">Product Quantity</label>
                <input class="form-control" value="<?= get_var('product_qty', $row[0]->product_qty)?>"type="text" name="product_qty" placeholder="Product Quantity">
            </div>
            <div class="mb-2">
                <label for="" class="text-white">Product Type</label>
                <input class="form-control" value="<?= get_var('product_type', $row[0]->product_type)?>"type="text" name="product_type" placeholder="Product Type">
            </div>
            <div class="d-grid gap-2 col-6 mx-auto">
                <button type="submit" class="btn bg-black bg-gradient text-white">Update Product</button>
